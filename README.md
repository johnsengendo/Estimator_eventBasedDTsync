...
| **Lyapunov** | keep a virtual queue `Z_{k+1} = max{0, Z_k + s_k − r_q}` and choose `s_k` by minimizing a drift-plus-penalty term.
...
clean daily rhythm, `ρ → 0` for transient-dominated traffic. 
